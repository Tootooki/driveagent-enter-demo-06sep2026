...
Promise.all([fetch('sheet-data.json').then(r=>r.json()),fetch('sheet-layout.json').then(r=>r.json())]).then(([sheet,layout])=>{
  ...
  const render=()=>{
    ...
    if(!filteredRows.length){const empty=document.createElement('p');empty.className='accounting-empty';empty.setAttribute('role','status');empty.textContent='No matching products';mount.append(empty);}
  };
