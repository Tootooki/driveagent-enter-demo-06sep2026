...
import {TABS, CONFIG, createWorkspaceState, selectWorkspace, setOption, setWorkspaceQuery, viewFor} from './workspace-model.mjs?v=105';
...
const tabButtons=TABS.map(label=>{
  const id=label.toLowerCase(),el=button(tabLabel(id),()=>chooseTab(id),'workspace-tab');
...
document.addEventListener('keydown',event=>{
  if(event.defaultPrevented||event.key!=='Escape'||event.target.closest?.('#chat-widget')||ppcPanel.hidden||!backdrop.hidden||document.querySelector('dialog[open]'))return;
...
function paintWorkspace(){
  ...
  workspace.classList.toggle('is-module-sheet',!accounting&&state.active!=='ppc');
  document.title=`DRIVE AGENT — ${state.active==='ppc'?state.options.ppc.view:tabLabel(state.active)}`;
  workspace.removeAttribute('aria-labelledby');workspace.setAttribute('aria-label',`${tabLabel(state.active)} sheet`);
  if(!accounting){
